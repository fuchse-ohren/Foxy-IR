...
function airconCard(card, appliance) {
  ...
  const power = setting.button == "power-off" ? "切" : "入";
  ...
  card.append(make("p", `現在設定: 電源 ${power} / モード ${mode} / 温度 ${temp} / 風量 ${volume || "auto"} / 風向 ${dir || "auto"}`));
  ...
  const items = [
    ...
    [
      "温度−",
      () => {
        tempInput.value = Number(tempInput.value || temp || 0) - 1;
        return airconOkuru(appliance, { temperature: tempInput.value });
      },
    ],
    [
      "温度＋",
    ...
    ["運転モード切替", () => airconOkuru(appliance, { operation_mode: nextValue(modeList, mode, 1) })],
  ...
  items.forEach(([label, fn]) => {
    const b = make("button", label, "dads-button control-button");
    b.dataset.size = "md";
    b.dataset.type = "outline";
...
function signalButtons(card, appliance, signals) {
  ...
  signals.forEach((signal) => {
    const b = make("button", signal.name || signal.id, "dads-button control-button");
    b.dataset.size = "md";
    b.dataset.type = "outline";
    b.addEventListener("click", async () => {
      await signalOkuru(signal.id);
      msgDasu(`${signal.name || signal.id} を送信しました。`);
    });
    row.append(b);
  });
  card.append(row);
...
async function main() {
  try {
    ...
    appliances.forEach((appliance) => {
      ...
      // カードに名前を追加
      const cardTitle = make("h2", appliance.nickname || appliance.id);
      // 名前の横にリフレッシュボダンを追加
      const fresh = make("button", "", "refresh-button");
      fresh.addEventListener("click", async () => {
        await cacheRefresh(appliance.id);
        await main();
      });

      cardTitle.append(fresh);
